...
#             base_preds.append( e.predict_proba(X) )
#     fbar = np.mean(base_preds,axis=0)
#     return hinge_loss(target, fbar)

def diversity(model, X, target): 
    #eye_matrix = torch.eye(n_classes).repeat(n_preds, 1, 1).cuda()
    # print("Computing base_preds in diversity")
    base_preds = []
    if hasattr(model, "weights_"):
        for e,w in zip(model.estimators_, model.weights_):
            base_preds.append( len(model.estimators_) * w * e.predict_proba(X) )
    ...
    diversities = []
    for hpreds in base_preds:
        hdiv = []
        for h, f in zip(hpreds, fbar):
            d = h - f
            D = 2 * 1.0 / model.n_classes_ * np.eye(model.n_classes_)
            tmp = np.matmul(d, np.matmul(D, d))
            hdiv.append(tmp)
        
        diversities.append( 1.0 / len(model.estimators_) * 1.0/2.0 * np.mean(hdiv) )
    return np.sum(diversities)

    # D = 2.0 * np.repeat(np.eye(model.n_classes_)[np.newaxis,:,:], X.shape[0], axis=0)
    # for pred in base_preds:
    #     diff = pred - fbar 
    #     # print(D.shape)
    #     # print(diff.shape)
    #     tmp = np.matmul(D,diff[:,:,None]).squeeze()
    #     covar = np.sum(tmp * diff[:,None],axis=(1,2))
    #     #covar = np.bmm(diff.unsqueeze(1), np.bmm(D, diff.unsqueeze(2))).squeeze()
    #     div = 1.0 / model.n_classes_ * 1.0/model.n_estimators * 0.5 * covar
    #     diversities.append(div)
    # diversities = np.array(diversities)
    # # print(diversities.shape)
    # return np.mean(np.sum(diversities,axis=0))   
...
def eval_model(X, Y, idx, model, rfs = None, use_prune = False, eval_bases = False):
    ...
    for i, (itrain, itest) in enumerate(idx):
        ...
        # metrics = {
        #     **min_max_accuracy(imodel, XTrain, YTrain, "train"),
        #     **metrics
        # }
    ...
    results = {
        ...
        # "train_bias":np.mean(metrics["train_bias"]),
        # "train_diversity":np.mean(metrics["train_diversity"]),
...
def main(args):
    ...
    for dataset in args.dataset:
        ...
        for h in args.height:
            ...
            # print("Training initial ETs with h = {}".format(h))
            # et = ExtraTreesClassifier(n_estimators = args.n_estimators, bootstrap = True, max_depth = h, n_jobs = args.n_jobs)
            # results, ets = eval_model(X,Y,idx,et,None,args.use_prune,True)
            # metrics.append(
            #     {
            #         **results,
            #         "height":h,
            ...
            # print("Training initial OverSampleForests with h = {}".format(h))
            # of = OversampleForest(n_estimators = args.n_estimators, bootstrap = True, max_depth = h, n_jobs = args.n_jobs, max_samples=1.0)
            # results, ofs = eval_model(X,Y,idx,of,None,args.use_prune,True)
            # metrics.append(
            #     {
            #         **results,
            #         "height":h,
